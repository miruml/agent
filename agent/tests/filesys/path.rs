...
// test cases taken from path-clean crate https://github.com/danreeves/path-clean/blob/3876d7cb5367997bcda17ce165bf69c4f434cb93/src/lib.rs#L108
    ...
    fn replace_multiple_slashes() {
        ...
        let parent_dir_path = path::clean(current_dir.parent().unwrap().path()).to_string_lossy().into_owned();

        let rel_path_to_thing_path = current_dir_path_buf.join("path/to/thing").to_string_lossy().into_owned();
    ...
    fn eliminate_current_dir() {
        ...
        let rel_test_path = current_dir_path_buf.join("test").to_string_lossy().into_owned();
        let rel_test_path_path = current_dir_path_buf.join("test/path").to_string_lossy().into_owned();
        ...
        for test in tests {
            assert_eq!(Dir::new(PathBuf::from(test.0)).abs_path().unwrap(), PathBuf::from(test.1));
        }
    }
    ...
    fn eliminate_parent_dir() {
        ...
        let parent_dir_path = path::clean(parent_dir_path_buf).to_string_lossy().into_owned();
        let grandparent_dir_path = path::clean(current_dir.parent().unwrap().parent().unwrap().path()).to_string_lossy().into_owned();

        let rel_test_path = current_dir_path_buf.join("test").to_string_lossy().into_owned();
        let rel_path_path = current_dir_path_buf.join("path").to_string_lossy().into_owned();
        let rel_another_path = current_dir_path_buf.join("another").to_string_lossy().into_owned();
        let rel_another_path_path = current_dir_path_buf.join("another/path").to_string_lossy().into_owned();
        let rel_parent_test_path = parent_dir_path_buf.join("test").to_string_lossy().into_owned();
        let rel_parent_test_path_path = parent_dir_path_buf.join("test/path").to_string_lossy().into_owned();
