    ...
    R: Read<ConfigInstanceID, serde_json::Value>,
{
    ...
    let (to_deploy, result) = remove_many(
        to_deploy, deployment_dir, settings, observers
    ).await;
    if let Err(e) = result {
    ...
    let (to_remove, result) = deploy_many(
        to_remove, cfg_inst_data_reader, deployment_dir, settings, observers
    ).await;
    if let Err(e) = result {
...
) -> (Vec<ConfigInstance>, Result<(), DeployErr>)
{
    ...
    let mut instances_iter = instances.into_iter();
    while let Some(instance) = instances_iter.next() {
        let (post_remove_instance, result) = remove(
            instance, deployment_dir, settings, observers
        ).await;
        if let Err(e) = result {
