...
// internal crates
use crate::{filesys::{errors::*, file::File}, trace, utils::Mergeable};
...
pub trait ConcurrentContentT<UpdatesT>: Clone + Serialize + DeserializeOwned + Mergeable<UpdatesT> + Send + Sync + 'static {}
impl<T, U> ConcurrentContentT<U> for T where T: Clone + Serialize + DeserializeOwned + Mergeable<U> + Send + Sync + 'static {}
    ...
    pub async fn run(mut self) {
        while let Some(cmd) = self.receiver.recv().await {
            match cmd {
                ...
                WorkerCommand::Patch { updates, respond_to } => {
    ...
    ContentT: ConcurrentContentT<UpdatesT>,
{
    ...
    pub async fn shutdown(&self) -> Result<(), FileSysErr> {
        ...
        info!("{} cached file shutdown complete", std::any::type_name::<ContentT>());
        Ok(())
    }
    ...
    pub async fn write(&self, data: ContentT) -> Result<(), FileSysErr> {
        let (send, recv) = oneshot::channel();
        self.sender
            .send(WorkerCommand::Write { data, respond_to: send })
            .await
            .map_err(|e| {
    ...
    pub async fn patch(&self, updates: UpdatesT) -> Result<(), FileSysErr> {
        let (send, recv) = oneshot::channel();
        self.sender
            .send(WorkerCommand::Patch { updates, respond_to: send })
            .await
            .map_err(|e| {
