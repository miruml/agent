...
pub async fn get_latest_concrete_config(configuration: &configuration::Configuration, config_schema_digest: &str, config_schema_slug: &str) -> Result<models::BaseConcreteConfig, Error<GetLatestConcreteConfigError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_config_schema_digest = config_schema_digest;
    let p_config_schema_slug = config_schema_slug;

    let uri_str = format!("{}/concrete_configs/latest", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);

    req_builder = req_builder.query(&[("config_schema_digest", &p_config_schema_digest.to_string())]);
    req_builder = req_builder.query(&[("config_schema_slug", &p_config_schema_slug.to_string())]);
