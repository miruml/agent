...
curl -fsSL https://raw.githubusercontent.com/miruml/agent/"$BRANCH"/scripts/install/install.sh | sh -s -- --debug="$DEBUG" --prerelease="$PRERELEASE" --git-branch="$BRANCH"  --backend-base-url="$BACKEND_BASE_URL"
