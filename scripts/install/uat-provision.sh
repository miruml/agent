...
BACKEND_HOST=$(backend_host --default="https://uat.api.miruml.com" "$@")
if [ "$DEBUG" = true ]; then
...
if [ "$DEBUG" = true ]; then
    print_allow_reactivation_flag "$ALLOW_REACTIVATION"
fi

DEBUG=$(debug_flag "$@")
if [ "$DEBUG" = true ]; then
    debug "Script: uat-provision.sh"
fi
BRANCH=$(git_branch "$@")
if [ "$DEBUG" = true ]; then
...
MIRU_API_KEY=$MIRU_API_KEY curl -fsSL https://raw.githubusercontent.com/miruml/agent/"$BRANCH"/scripts/install/provision.sh | sh -s -- \
--backend-host="$BACKEND_HOST" \
--device-name="$DEVICE_NAME" \
--allow-reactivation="$ALLOW_REACTIVATION" \
--debug="$DEBUG" \
--git-branch="$BRANCH" \
--prerelease="$PRERELEASE" \
--install-script-file="uat-install.sh" \
--version="$VERSION"
